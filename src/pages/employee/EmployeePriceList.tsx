...
const EmployeePriceList = () => {
  const { tuckshopId, permissions } = useAuth();
  ...
  const canManagePrice = userPermissions?.manage_price_list;
          ...
          <Table className="premium-table">
            <TableHeader className="bg-muted/50">
              <TableRow>
                ...
                {canManagePrice && <TableHead className="text-right">Actions</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                <TableRow key={item.id} className="group transition-colors hover:bg-muted/30">
                  ...
                  {canManagePrice && (
                    <TableCell className="text-right space-x-1">
                      <Button size="icon" variant="ghost" onClick={() => openEdit(item)} className="h-8 w-8 hover:bg-primary/10 hover:text-primary">
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button size="icon" variant="ghost" onClick={() => handleDelete(item.id)} className="h-8 w-8 hover:bg-destructive/10 hover:text-destructive">
              ...
              {filteredItems.length === 0 && (
                <TableRow>
                  <TableCell colSpan={canManagePrice ? 4 : 3} className="text-center text-muted-foreground py-12">
                    {searchQuery ? (
...
export default EmployeePriceList;
