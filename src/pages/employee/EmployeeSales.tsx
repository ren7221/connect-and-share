...
import { Plus, Pencil, Trash2, AlertCircle, CalendarIcon, Lock, CheckCircle2, Package, Clock, User } from "lucide-react";
...
const EmployeeSales = () => {
  const { tuckshopId, user, permissions } = useAuth();
  ...
  // Permission guards
  const canEdit = (permissions?.edit_suppliers || (permissions as any)?.update_supply_sales) ?? false;
  const canManagePayments = permissions?.manage_payments ?? false;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          {!canEdit && <Badge variant="secondary" className="mt-1 w-fit font-medium">View Only</Badge>}
        </div>
      ...
      <div className="grid gap-4 md:hidden">
        {sales.map((s) => {
          ...
          const np = ts * 0.88;
          return (
            <Card key={s.id} className={cn("overflow-hidden", s.payment_locked && "bg-muted/10")}>
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="text-base font-bold text-primary">{s.commodity_name}</CardTitle>
                    <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1"><User className="h-3 w-3" />{s.supplier_name}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {canEdit && !s.payment_locked && (
                      <>
                        <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => openEdit(s)}><Pencil className="h-3 w-3" /></Button>
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => handleDelete(s.id, !!s.payment_locked)}><Trash2 className="h-3 w-3" /></Button>
                      </>
                    )}
                  </div>
                </div>
              ...
              <CardContent className="grid grid-cols-2 gap-3 pt-0">
                <div><p className="text-[10px] uppercase font-bold text-muted-foreground">Price</p><p className="text-sm font-bold">{fmt(s.unit_price)}</p></div>
                <div className="text-right"><p className="text-[10px] uppercase font-bold text-muted-foreground">Brought / Sold</p><p className="text-sm font-bold">{fmtNum(s.quantity_supplied)} <span className="text-muted-foreground">/ {fmtNum(s.quantity_sold)}</span></p></div>
                <div><p className="text-[10px] uppercase font-bold text-muted-foreground">Total Sales</p><p className="text-sm font-bold">{fmt(ts)}</p></div>
                <div className="text-right"><p className="text-[10px] uppercase font-bold text-muted-foreground">Net Payable</p><p className="text-sm font-bold text-primary">{fmt(np)}</p></div>
                <div className="col-span-2 flex justify-between items-center border-t pt-3">
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Clock className="h-3 w-3" />
                    <span>{s.sale_date}</span>
                    {s.supply_time && <span>· {new Date(s.supply_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
                  </div>
                  <div className="flex items-center gap-2">
                    {(canManagePayments || canEdit) && (
                      <Switch
                        checked={s.is_paid}
                        disabled={s.payment_locked || !(canManagePayments || canEdit)}
                        onCheckedChange={(c) => togglePaidInitiate(s, c)}
                      />
                    )}
                    {s.payment_locked && <Lock className="h-3 w-3 text-muted-foreground" />}
                    {s.is_paid ? <Badge variant="default" className="text-[10px] h-5">Paid</Badge> : <Badge variant="destructive" className="text-[10px] h-5">Pending</Badge>}
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        {sales.length === 0 && (
          <Card className="border-dashed flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
            ...
            <p className="font-medium">No supply records found for selected period.</p>
          </Card>
        )}
      </div>

      {/* Desktop Table */}
      <div className="hidden md:block responsive-table-container shadow-xl shadow-black/5 rounded-xl border bg-card overflow-hidden">
        <Table className="premium-table w-full whitespace-nowrap">
          ...
          <TableBody>
            ...
            {sales.length === 0 && <TableRow><TableCell colSpan={canEdit ? 10 : 9} className="text-center text-muted-foreground py-8">No supply records found for selected period.</TableCell></TableRow>}
          </TableBody>
        </Table>
      </div>
